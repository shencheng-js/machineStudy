...
def gaosi(x, x_i, k=0.4):  # k值是σ值，0.5,越大影响范围越小
...
# 用高斯就用1.65的门槛
def getjihuo(yuan, activlis, threshold=2.64, fre=1):  # 这里需要控制量值，也就是多大刺激能够激活,以及其之前的影响，默认为没有
    temp = [0] * 100
    for i in range(0, 100):
        for j in range(0, 100):
...
if __name__ == "__main__":
    ...
    for i in range(100):  # 训练100次
        for j in range(0, 100):
            if activation_list[j] == 1:  # 如果是被激活的，刺激其他神经元
                accumulation = activ(accumulation, j)  # 一次激活完成

        # print(yuan)
        # 一轮训练激活结束，找成功激活的神经元
        activation_list = getjihuo(accumulation, activlis=activation_list)  # 默认之前的无影响
        # 随机激活几个神经元
    ...
    #         yuan = getzero()
    #
    #     plot_result(data=result, time=time)
